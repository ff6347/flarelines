...
3. **Connected the dots:**
   ...
   - `deleteStoreIfIncompatible()` detected metadata mismatch → deleted all user data
...
## Additional: Simplified Beads Workflow

Removed `bd sync --from-main` from the workflow - it was creating annoying worktrees at `.git/beads-worktrees/main`. For a solo developer project where only Claude manages beads, no special sync is needed. The JSONL file is just committed alongside code changes.

## Session Status

- PR #22 merged
- Build archived and uploaded to App Store Connect
- Beads workflow simplified in AGENTS.md
